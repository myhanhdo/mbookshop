...
function Product({ match, history }) {
  ...
  useEffect(() => {
    ...
    TweenMax.from(imgRef, 1, {
      opacity: 0,
      x: -30,
      ease: Power3.easeOut,
      delay: 0.5,
    });
    TweenMax.from(nameRef, 1, {
      opacity: 0,
      y: 30,
      ease: Power3.easeOut,
      delay: 1,
    });
    TweenMax.from(authorRef, 1, {
      opacity: 0,
      y: 30,
      ease: Power3.easeOut,
      delay: 1.1,
    });
    TweenMax.from(divARef, 1, {
      opacity: 0,
      y: 30,
      ease: Power3.easeOut,
      delay: 1.1,
    });
    TweenMax.from(genreRef, 1, {
      opacity: 0,
      y: 30,
      ease: Power3.easeOut,
      delay: 1.2,
    });
    TweenMax.from(divBRef, 1, {
      opacity: 0,
      y: 30,
      ease: Power3.easeOut,
      delay: 1.2,
    });
    TweenMax.from(pubRef, 1, {
      opacity: 0,
      y: 30,
      ease: Power3.easeOut,
      delay: 1.3,
    });
    TweenMax.from(divCRef, 1, {
      opacity: 0,
      y: 30,
      ease: Power3.easeOut,
      delay: 1.3,
    });
    TweenMax.from(ratingRef, 1, {
      opacity: 0,
      y: 30,
      ease: Power3.easeOut,
      delay: 1.4,
    });
    TweenMax.from(descRef, 1, {
      opacity: 0,
      y: 30,
      ease: Power3.easeOut,
      delay: 1.5,
    });
    TweenMax.from(priceRef, 1, {
      opacity: 0,
      x: 40,
      ease: Power3.easeOut,
      delay: 1.3,
    });
    TweenMax.from(reviewsRef, 1, {
      opacity: 0,
      ease: Power3.easeOut,
      delay: 1.7,
    });
          ...
          >
            <ImgAndInfo>
              ...
              <InfoContainer component="div">
                ...
                <p ref={(el) => (pubRef = el)}>
                  Publication: <strong>{product.publication}</strong>
                </p>
                <Divider ref={(el) => (divCRef = el)} />
                <RatingContainer ref={(el) => (ratingRef = el)}>
                  <Rating name="read-only" value={product.rating} readOnly />
                  <span className="num-review">{`from ${product.numReviews} review`}</span>
                </RatingContainer>
              </InfoContainer>
            </ImgAndInfo>
